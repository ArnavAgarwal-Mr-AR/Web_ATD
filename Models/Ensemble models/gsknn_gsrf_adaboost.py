...
# Create base models (KNN and Random Forest)
knn_model = grid_search_knn.best_estimator_
rf_model = grid_search_rf.best_estimator_

# Create a list of base models
base_models = [knn_model, rf_model]

# Create an AdaBoostClassifier with base models
adaboost_classifier = AdaBoostClassifier(base_estimator=rf_model, n_estimators=50, learning_rate=1.0, random_state=42)  # You can adjust parameters here

# Start the timer
start_timer()

# Train the AdaBoost ensemble model on the resampled training data
adaboost_classifier.fit(X_train_resampled, y_train_resampled)

# Stop the timer and save the training time
training_time = stop_timer("ENS_AdaBoost_gKNN_gRF")
...
save_accuracies("ENS_AdaBoost_gKNN_gRF", training_accuracy, validation_accuracy, testing_accuracy, training_classification_rep, validation_classification_rep, testing_classification_rep)
