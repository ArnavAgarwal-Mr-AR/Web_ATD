#GRidsearched KNN and Gridsearched Random Forest by Stacking
from sklearn.ensemble import StackingClassifier
from sklearn.metrics import accuracy_score, classification_report

# Create a stacking ensemble of KNN and Random Forest models
estimators = [
...
stacking_classifier = StackingClassifier(
    ...
    final_estimator=best_rf_model  # You can choose any model as the final estimator
)

# Start the timer
start_timer()

# Train the stacking ensemble on the resampled training data
stacking_classifier.fit(X_train_resampled, y_train_resampled)

# Stop the timer and save the training time
training_time = stop_timer("STACKING_gKNN_gRF")

# Predict on training, validation, and testing sets
y_train_pred = stacking_classifier.predict(X_train_scaled)
y_val_pred = stacking_classifier.predict(X_val_scaled)
y_test_pred = stacking_classifier.predict(X_test_scaled)
...
save_accuracies("STACKING_gKNN_gRF", training_accuracy, validation_accuracy, testing_accuracy, training_classification_rep, validation_classification_rep, testing_classification_rep)
